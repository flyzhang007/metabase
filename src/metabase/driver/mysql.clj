...
(defn- timezone-id->offset-str
  ...
  [^String timezone-id]
  (when timezone-id
    (time/unparse (time/formatter "ZZ" (t/time-zone-for-id timezone-id)) (t/now))))
...
(defmethod sqlqp/->honeysql [MySQLDriver Date]
  [_ date]
  (let [timestamp-literal          (hx/literal (u/format-date :date-hour-minute-second-ms date))
        report-timezone-offset-str (timezone-id->offset-str (driver/report-timezone))
        system-timezone-offset-str (timezone-id->offset-str (.getID (TimeZone/getDefault)))]
    (if (and report-timezone-offset-str
      ...
      ;; otherwise if we don't have a report timezone just we just want SQL like timestamp('2004-01-01T12:00:00')
      (hsql/call :timestamp timestamp-literal))))
